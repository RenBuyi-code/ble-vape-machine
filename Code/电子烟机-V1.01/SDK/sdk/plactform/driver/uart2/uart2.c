...
#define uart2_rx_en    0x0      // 0: Disable RX, 1: Enable RX
...
#define UART2_READ_BYTE()                 ((REG_APB3_UART2_PORT>>8)&0xff)
...
void uart2_init(uint32_t baudrate)
{
	...
	REG_APB5_GPIOB_CFG  &= ~((0x40<<BIT_GPIO_PULL_UP)  + (0x40<<BIT_GPIO_PERI_EN));
	REG_APB5_GPIOB_CFG  |= ((0x40<<BIT_GPIO_PULL_UP));
	REG_APB5_GPIOB_CFG  |=   (0x40<<BIT_GPIO_OUT_EN_N);

	REG_APB5_GPIOB_DATA &= ~ (0x40<<BIT_GPIO_INPUT_EN);


	uart2_env.rx.callback = NULL;
	uart2_env.rx.dummy    = NULL;

	//REG_AHB0_ICU_INT_ENABLE |= (0x01 << 16); 不开中断
