...
void Write_NVR_Flash(uint32_t address, uint8_t *buffer,uint32_t length)
{
	...
	flash_erase_sector(FLASH_SPACE_TYPE_NVR,address); //²Á³ýºóÐ´Èë
	flash_write(FLASH_SPACE_TYPE_NVR,address,length,buffer);
}

void Read_NVR_Flash(uint32_t address, uint8_t *buffer,uint32_t length)
{
		flash_read(FLASH_SPACE_TYPE_NVR,address,length,buffer);
