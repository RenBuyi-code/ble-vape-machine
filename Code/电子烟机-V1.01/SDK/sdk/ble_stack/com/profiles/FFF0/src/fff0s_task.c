                                      ...
                                      ke_task_id_t const dest_id, ke_task_id_t const src_id)
{
    ...
    uint8_t status = fff0s_get_att_idx(param->handle,  &att_idx);
	  ...
	  UART_PRINTF("gattc_write_req_ind_handler  att_idx = %d \r\n",att_idx);			
    ...
    if(status == GAP_ERR_NO_ERROR)
    {
		...
		{
			UART_PRINTF("----------------FFF0S_IDX_FFF1_LVL_VAL\r\n ");			
			
			// Allocate the alert value change indication   FFF0S_FFF2_WRITER_REQ_IND 改了这里
