...
void app_fff0_add_fff0s(void)
{
    ...
    struct gapm_profile_task_add_cmd *req =
        app_profile_add_cmd_alloc(TASK_ID_FFF0S, sizeof(struct fff0s_db_cfg));
...
static int fff0s_fff1_level_ntf_cfg_ind_handler(ke_msg_id_t const msgid,
        ...
        ke_task_id_t const src_id) // ntf 处理函数 就是ntf被触发
{
    UART_PRINTF("param->ntf_cfg = %x\r\n",param->ntf_cfg);
    UART_PRINTF("fff0s_fff1_level_ntf_cfg_ind_handler \r\n ");
    UART_PRINTF("param->conidx = %x\r\n",param->conidx);

    if(param->ntf_cfg == PRF_CLI_STOP_NTFIND)
    {
        ke_timer_clear(APP_BATTERY_TIMER, TASK_APP);				//关闭上报电量定时器
        ke_timer_clear(FFF0S_FFF1_LEVEL_PERIOD_NTF,dest_id);
    }
    else //打开ntf 自动上报电量 1秒一次
    {
